 ...
 * RoFormerEngine
 *
 * Runs BS-RoFormer / Mel-Band RoFormer models with rotary position embeddings for fine harmonic vocal isolation.
 */
class RoFormerEngine(
    private val modelSpec: ModelSpec = ModelRegistry.ROFORMER_MELBAND
) : SourceSeparationEngine {

    override val engineName: String = "Mel-Band RoFormer (${modelSpec.name})"

    override val isAvailable: Boolean
        get() = ModelManager.isModelAvailable(modelSpec, context)

    companion object {
        private const val TAG = "RoFormerEngine"
    }
    ...
    override suspend fun separate(
        ...
        if (!isAvailable || !ModelManager.verifyModelIntegrity(modelSpec, context)) {
            Log.w(TAG, "RoFormer ONNX model not ready, falling back to Demucs/Native DSP...")
            return@withContext DemucsEngine().separate(input, config, onProgress)
        }

        onProgress?.invoke(0.05f, "تهيئة نموذج Mel-Band RoFormer...")
        ...
        val session = try {
            ...
            Log.e(TAG, "Failed to initialize RoFormer session", e)
            return@withContext DemucsEngine().separate(input, config, onProgress)
        }
        ...
        try {
            ...
            val (outLeft, outRight) = AudioChunkProcessor.processStreaming(
                ...
                onProgress = { p ->
                    onProgress?.invoke(0.10f + p * 0.70f, "فصل عبر RoFormer: ${(p * 100).toInt()}%")
                }
