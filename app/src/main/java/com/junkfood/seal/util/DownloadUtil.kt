...
object DownloadUtil {
    ...
    const val PLAYLIST_INDEX_PADDED = "%(playlist_index&{:0>3d})s"
                    ...
                    val output =
                        if (splitByChapter) {
                            ...
                            if (downloadPlaylist && playlistNumbering) {
                                if (outputTemplate == OUTPUT_TEMPLATE_ID) OUTPUT_TEMPLATE_PLAYLIST_ID
                                else if (outputTemplate == OUTPUT_TEMPLATE_DEFAULT) OUTPUT_TEMPLATE_PLAYLIST
                                else outputTemplate
                            } else {
                                outputTemplate
                            }
