...
interface VideoInfoDao {
    ...
    @Insert suspend fun insertDownloadOperation(operation: DownloadOperation): Long

    @Update suspend fun updateDownloadOperation(operation: DownloadOperation)

    @Delete suspend fun deleteDownloadOperation(operation: DownloadOperation)

    @Query("SELECT * FROM download_operation ORDER BY timestamp DESC")
    fun getDownloadOperationsFlow(): Flow<List<DownloadOperation>>

    @Query("DELETE FROM download_operation")
