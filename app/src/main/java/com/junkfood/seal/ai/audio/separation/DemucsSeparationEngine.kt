...
/**
 * DemucsSeparationEngine
 *
 * Runs Meta's Demucs v4 hybrid neural network via ONNX Runtime.
 */
class DemucsSeparationEngine(
    private val modelSpec: ModelSpec = ModelRegistry.DEMUCS_V4_VOCALS
) : AudioSeparationEngine {

    override val engineName: String = "Demucs v4 Engine (${modelSpec.name})"

    companion object {
        private const val TAG = "DemucsSeparationEngine"
    }
    ...
    override suspend fun separate(
        ...
        if (!modelFile.exists() || modelFile.length() < 1024L) {
            Log.w(TAG, "Demucs ONNX model not downloaded, falling back to MDX/DSP...")
            return@withContext NativeDspFallbackSeparationEngine.separate(input, options, onProgress)
        }

        onProgress?.invoke(0.05f, "جاري تهيئة نموذج Demucs v4 بالذكاء الاصطناعي...")
        ...
        val session = try {
            ...
            Log.e(TAG, "Failed to initialize Demucs session", e)
            return@withContext NativeDspFallbackSeparationEngine.separate(input, options, onProgress)
        }
        ...
        try {
            ...
            val (procLeft, procRight) = OverlapAddProcessor.processStereo(
                ...
                onProgress = { p ->
                    onProgress?.invoke(0.1f + p * 0.75f, "فصل عميق عبر Demucs: ${(p * 100).toInt()}%")
                }
                ...
                inputTensor.use { tensor ->
                    ...
                    result.use { res ->
                        ...
                        for (i in 0 until copyLen) outLeft[i] = outputBuffer.get()
                        for (i in 0 until copyLen) outRight[i] = outputBuffer.get()
                    }
                }
