    ...
    suspend fun downloadTracks(
        ...
        runCatching {
            ...
            try {
                ...
                val requestedLangCodes: Set<String> = if (preferences.subtitleLanguage.isBlank() || preferences.subtitleLanguage.equals("all", ignoreCase = true)) {
                    emptySet<String>()
                } else {
                    preferences.subtitleLanguage.split(',').map { LanguageMatcher.getBaseLanguageCode(it.trim()) }.filter { it.isNotEmpty() }.toSet()
                }
                val effectiveTempFiles = if (requestedLangCodes.isNotEmpty()) {
                    val matched = tempFiles.filter { file ->
                        val fileLang = Regex("""\.([a-zA-Z]{2,3})(?:-[a-zA-Z0-9_-]+)?\.[a-zA-Z0-9]+$""").find(file.name)?.groupValues?.get(1)?.lowercase(Locale.ROOT)
                        fileLang != null && requestedLangCodes.contains(fileLang)
                    }
                    if (matched.isNotEmpty()) matched else tempFiles
                } else tempFiles

                for (tempFile in effectiveTempFiles) {
                    // Deep Validation
    ...
    suspend fun downloadSubtitlesDirectly(
        ...
        runCatching {
            ...
            try {
                ...
                val requestedLangCodes: Set<String> = if (subLangs.isBlank() || subLangs.equals("all", ignoreCase = true)) {
                    emptySet<String>()
                } else {
                    subLangs.split(',').map { LanguageMatcher.getBaseLanguageCode(it.trim().removeSuffix(".*").removeSuffix("-*")) }.filter { it.isNotEmpty() }.toSet()
                }

                // If specific languages were requested, filter out unwanted languages (e.g. unwanted en-orig when user wanted ar)
                val filteredTempFiles = if (requestedLangCodes.isNotEmpty()) {
                    val matched = tempFiles.filter { file ->
                        val fileLang = Regex("""\.([a-zA-Z]{2,3})(?:-[a-zA-Z0-9_-]+)?\.[a-zA-Z0-9]+$""").find(file.name)?.groupValues?.get(1)?.lowercase(Locale.ROOT)
                        fileLang != null && requestedLangCodes.contains(fileLang)
                    }
                ...
                val deduplicatedTempFiles = filteredTempFiles
                    .groupBy { file ->
