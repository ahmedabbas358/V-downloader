...
data class Task(
    ...
    data class ViewState(
        ...
        val fileSizeApproxBytes: Long = 0L,
        ...
        companion object {
            fun fromVideoInfo(info: VideoInfo, preferences: DownloadUtil.DownloadPreferences? = null): ViewState {
                ...
                return ViewState(
                    ...
                    durationMs = if (isSubOnly) null else info.duration?.times(1000)?.toLong(),
                    thumbnailUrl = info.thumbnail.toHttpsUrl(),
                    fileSizeApproxBytes = if (isSubOnly) 0L else (info.fileSize ?: info.fileSizeApprox ?: .0).toLong(),
