...
data class VideoInfo(
    ...
    fun availableResolutions(): List<Int> =
        availableFormats()
            ...
            .mapNotNull { it.parsedHeight?.roundToInt() }
            .filter { it > 0 }
            .distinct()
            .sortedDescending()
    ...
    fun bestVideoOnlyFormat(targetHeight: Int? = null): Format? =
        availableFormats()
            ...
            .let { list -> targetHeight?.let { h -> list.filter { (it.parsedHeight?.roundToInt() ?: 0) <= h } } ?: list }
...
data class Format(
    ...
    val parsedWidth: Double?
        get() {
            if (width != null && width > 0.0) return width
            if (resolution != null) {
                val split = resolution.split("x")
                if (split.size == 2) return split[0].toDoubleOrNull()
            }
            return null
        }
    ...
    val parsedHeight: Double?
        get() {
            if (height != null && height > 0.0) return height
            if (resolution != null) {
                val match = Regex("(\\d+)[pP]").find(resolution)
                if (match != null) return match.groupValues[1].toDoubleOrNull()
                val split = resolution.split("x")
                if (split.size == 2) return split[1].toDoubleOrNull()
            }
            return null
        }
    /**
    ...
    fun containsVideo(): Boolean = !vcodec.isAbsentCodec() || (parsedWidth != null && parsedWidth!! > 0.0) || (parsedHeight != null && parsedHeight!! > 0.0) || (vcodec.isAbsentCodec() && acodec.isAbsentCodec() && resolution != "audio only")
